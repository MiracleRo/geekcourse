...
function layout(element) {
  ...
  if (mainSpace < 0) {
    ...
    flexLines.forEach((items) => {
      ...
      if (flexTotal < 0) {
        ...
        let currentMain;
        let step;
        if (itemStyle.justifyContent === 'flex-start') {
          currentMain = mainBase;
          step = 0;
        }
        if (itemStyle.justifyContent === 'flex-end') {
          currentMain = mainSpace * mainSign + mainBase;
          step = 0;
        }
        if (itemStyle.justifyContent === 'center') {
          currentMain = mainSpace / 2 * mainSign + mainBase;
          step = 0;
        }
        if (itemStyle.justifyContent === 'space-between') {
          currentMain = mainBase;
          step = mainSpace / (items.length - 1) * mainSign;
        }
        if (itemStyle.justifyContent === 'space-around') {
  ...
  flexLines.forEach((items) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item === null) {
        item[crossSize] = (align === 'stretch') ? lineCrossSize : 0;
      }
      if (align === 'flex-start') {
        itemStyle[crossStart] = crossBase;
        itemStyle[crossEnd] = itemStyle[crossStart] + crossSign * elementStyle[crossSize]
      }
      if (align === 'flex-start') {

        itemStyle[crossEnd] = crossBase + crossSign * lineCrossSize;
        itemStyle[crossStart] = itemStyle[end] - crossSign * elementStyle[crossSize]
      }
      if (align === 'center') {
